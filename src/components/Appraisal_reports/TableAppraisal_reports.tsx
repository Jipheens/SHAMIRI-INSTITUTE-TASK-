...
import { loadColumns } from './configureAppraisal_reportsCols';
import _ from 'lodash';
import dataFormatter from '../../helpers/dataFormatter';

const perPage = 5;

const TableSampleAppraisal_reports = ({
  filterItems,
  setFilterItems,
  filters,
}) => {
  ...
  const {
    ...
    loading,
    count,
    notify: appraisal_reportsNotify,
    refetch,
  } = useAppSelector((state) => state.appraisal_reports);
  ...
  useEffect(() => {
    if (appraisal_reportsNotify.showNotification) {
      notify(
        appraisal_reportsNotify.typeNotification,
        appraisal_reportsNotify.textNotification,
      );
    }
  }, [appraisal_reportsNotify.showNotification]);
  ...
  const handleEditAction = (id: string) => {
    router.push(`/appraisal_reports/${id}`);
  };

  const handleViewAction = (id: string) => {
    router.push(`/appraisal_reports/appraisal_reports-view/?id=${id}`);
  };
  ...
  console.log('appraisal_reports: ', appraisal_reports);
...
export default TableSampleAppraisal_reports;
