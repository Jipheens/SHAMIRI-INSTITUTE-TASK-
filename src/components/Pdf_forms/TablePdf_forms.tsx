...
import {
  ...
  deleteItem,
  setRefetch,
} from '../../stores/pdf_forms/pdf_formsSlice';
import { useAppDispatch, useAppSelector } from '../../stores/hooks';
import { useRouter } from 'next/router';
import { Field, Form, Formik } from 'formik';
import { DataGrid, GridColDef } from '@mui/x-data-grid';
import { loadColumns } from './configurePdf_formsCols';
import _ from 'lodash';
import dataFormatter from '../../helpers/dataFormatter';

const perPage = 5;

const TableSamplePdf_forms = ({ filterItems, setFilterItems, filters }) => {
  ...
  const {
    ...
    loading,
    count,
    notify: pdf_formsNotify,
    refetch,
  } = useAppSelector((state) => state.pdf_forms);
  ...
  useEffect(() => {
    if (pdf_formsNotify.showNotification) {
      notify(
        pdf_formsNotify.typeNotification,
        pdf_formsNotify.textNotification,
      );
    }
  }, [pdf_formsNotify.showNotification]);
  ...
  const handleEditAction = (id: string) => {
    router.push(`/pdf_forms/${id}`);
  };

  const handleViewAction = (id: string) => {
    router.push(`/pdf_forms/pdf_forms-view/?id=${id}`);
  };
  ...
  console.log('pdf_forms: ', pdf_forms);
...
export default TableSamplePdf_forms;
