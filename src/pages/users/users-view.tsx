    ...
    <>
      ...
      <SectionMain>
        ...
        <CardBox>
            ...
            >
              <div className='overflow-x-auto'>
                <table>
                  ...
                  <tbody>
                    {users.appraisal_reports_appraiser &&
                      Array.isArray(users.appraisal_reports_appraiser) &&
                      users.appraisal_reports_appraiser.map((item: any) => (
                        <tr
                          key={item.id}
                          onClick={() =>
                            router.push(
                              `/appraisal_reports/appraisal_reports-view/?id=${item.id}`,
                            )
                          }
                        >
                          <td data-label='report_number'>
                            {item.report_number}
                          </td>
              ...
              {!users?.appraisal_reports_appraiser?.length && (
                <div className={'text-center py-4'}>No data</div>
              )}
            </CardBox>
          </>
          ...
          <>
            <p className={'block font-bold mb-2'}>Pdf_forms Appraiser</p>
            <CardBox
              className='mb-6 border border-gray-300 rounded overflow-hidden'
              hasTable
            >
              <div className='overflow-x-auto'>
                <table>
                  ...
                  <tbody>
                    {users.pdf_forms_appraiser &&
                      Array.isArray(users.pdf_forms_appraiser) &&
                      users.pdf_forms_appraiser.map((item: any) => (
                        <tr
                          key={item.id}
                          onClick={() =>
                            router.push(
                              `/pdf_forms/pdf_forms-view/?id=${item.id}`,
                            )
                          }
                        >
                          <td data-label='form_name'>{item.form_name}</td>
                        </tr>
                      ))}
                  </tbody>
                </table>
              </div>
              {!users?.pdf_forms_appraiser?.length && (
                <div className={'text-center py-4'}>No data</div>
              )}
            </CardBox>
          </>
